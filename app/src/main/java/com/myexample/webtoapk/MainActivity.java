        ...
        try {
            ...
            QbSdk.setTbsListener(new QbSdk.TbsListener() {
                ...
                public void onDownloadFinish(int i) {
                    Log.d("WebToApk-X5", "onDownloadFinish: " + i);
                }

                @Override
                public void onInstallFinish(int i) {
                    Log.d("WebToApk-X5", "onInstallFinish: " + i);
                }

                @Override
                public void onDownloadProgress(int i) {
                    Log.d("WebToApk-X5", "onDownloadProgress: " + i);
                }
            });
            QbSdk.initX5Environment(getApplicationContext(), new QbSdk.PreInitCallback() {
                @Override
                public void onCoreInitFinished() {
                    Log.d("WebToApk-X5", "onCoreInitFinished");
                }

                @Override
                public void onViewInitFinished(boolean success) {
                    Log.d("WebToApk-X5", "onViewInitFinished, success=" + success);
                }
            });
        } catch (Throwable t) {
            Log.e("WebToApk-X5", "X5 init failed, fallback to system WebView", t);
        }
    }
