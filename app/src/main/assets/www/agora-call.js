...
(function () {
    ...
    function isRemoteContext() {
        return window.location.pathname.includes('remote');
    }
    ...
    function initUI() {
        ...
        // 5. 控制按钮
        const isRemotePage = window.location.pathname.includes('remote');
        console.log('🚀 [AgoraUI] 正在初始化控制面板, Mode:', isRemotePage ? 'Viewer/Remote' : 'Presenter');
    ...
    function updateMediaPermissionState(canMic, canVideo) {
        const isMaster = !window.location.pathname.includes('remote');
        if (isMaster) return;
    ...
    window.agoraModule = {
        ...
        toggleMic: async function () {
            if (window.location.pathname.includes('remote') && !isViewerMicAllowed) {
                alert('演示者已关闭观众麦克风权限');
                return;
            }
        ...
        toggleVideo: async function () {
            if (window.location.pathname.includes('remote') && !isViewerVideoAllowed) {
                alert('演示者已关闭观众摄像头权限');
                return;
            }
            if (isScreenSharing) {
        ...
        setMediaPermission: function(micAllowed, videoAllowed) {
            ...
            if (window.location.pathname.includes('remote') && !isViewerMicAllowed && localTracks.audio && localTracks.audio.enabled) {
                localTracks.audio.setEnabled(false).catch(() => {});
                updateMicBtn(false);
            }
            if (window.location.pathname.includes('remote') && !isViewerVideoAllowed && localTracks.camera && localTracks.camera.enabled) {
